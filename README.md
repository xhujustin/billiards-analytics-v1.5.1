...
### 球體顏色自動校正與追蹤提升 (v1.5.2 新增)

**顏色校正精靈與 K-Means 分析**
- 提供單顆球體引導式校正精靈，即時配對並調整每種顏色的 HSV 範圍。
- 後端導入 K-Means (K=3) 機器學習演算法提取 ROI 主色 (Dominant Color)，自動排除高光反光與邊緣陰影干擾。
- 具備直覺的設定總覽面板，點擊顏色方塊即可隨時跳轉返回編輯。

**YOLO 備援與容錯優化**
- 新增 Second-Pass Fallback 機制：當球數低於設定閾值時，自動以更高解析度 (`imgsz=960`) 與更低信心度 (`conf=0.04`) 重新推論，大幅降低漏偵測率。
- 放寬球體長寬比判定 (Aspect Ratio `0.50~1.90`)，有效改善快速移動時的殘影辨識成功率。
- 支援透過 `.env` 完整配置備援機制 (Second Pass) 與自動畫質適應功能 (`ENABLE_ADAPTIVE_QUALITY`)。
