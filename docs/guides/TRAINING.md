...
一般練習中，若目標球進袋後從 YOLO 結果消失，後端會以 planner 目標球號與上一幀彩球快照確認該球已進袋，不再被附近其他彩球覆蓋。

### 規範用法

- 一般練習成功次數以後端自動偵測為權威來源，前端只輪詢 `/api/practice/state` 顯示統計。
- 已知 `target_ball_number` 時，後續追蹤優先用球號匹配；球號消失且上一幀位置位於進袋接近區，或該幀洞口資料漏檢但目標球連續消失時，進入 missing/potted 確認。
- 成功條件維持為子球進袋且母球未進袋。
- 詳細輸出格式與 shot event 規範見 `docs/guides/PRACTICE_AUTO_DETECTION.md`。
