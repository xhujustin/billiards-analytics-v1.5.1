...
## 06/27: '修正無實心球時條紋球被硬改成實心'

### 功能說明
- 收斂同色滿色/條紋 pair constraint，避免檯面上沒有同色實心球證據時，將條紋球強制改成實心球號。
- 約束現在必須同時滿足條紋端與實心端各自有分數優勢，且不可覆蓋已有強條紋或強實心證據的候選球。
- 這項修正保留同色兩顆明確存在時的 `{實心, 條紋}` 補正能力，例如黃球仍可在證據足夠時分配為 `{1, 9}`。

### 規範用法
- 只剩條紋球或實心球已下袋時，不得用同色球數量平衡硬補不存在的實心球。
- 若同色候選全部已被可靠判定為 `Stripe`，`_apply_same_color_pair_constraints()` 應保持原本的 `style/number`。
- 若同色候選一顆有明確滿色特徵、另一顆有明確白帶特徵，才可套用 pair constraint。
