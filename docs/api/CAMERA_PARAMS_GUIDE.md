...
**顯示規則**:
- 點擊按鈕後在原區塊下方展開，不進入新頁面。
- 展開內容第一層為相機即時預覽。
- 相機參數調整控制必須排列在預覽下方。
- 參數調整區以「參數分類」下拉選單切換，一次只顯示一個分類面板。
- 再次點擊按鈕時收合內嵌內容。

**參數分類選項**:
- 曝光設定：曝光時間、ISO 感光度。
- 軟體降噪：啟用降噪、降噪強度、降噪演算法。
- 影像調整：亮度、對比度、飽和度。
- 白平衡：自動白平衡、色溫。
- 自動調整：自動調整所有參數。
...
**輸出格式**:
- 預覽串流來源維持 `/burnin/camera1.mjpg?quality=med&client_id=camera-params-monitor`。
- 參數更新仍使用 `POST /api/camera/params`，單次送出 `{ [key]: value }`。
