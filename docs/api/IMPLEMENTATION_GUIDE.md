...
- 統計頁不再使用 numbered badge；分類名稱集中於頂部標題按鈕列。
- 移除獨立「時間範圍」區段，頂部改為五個分類標題按鈕；最右側保留時間下拉選單，選擇後沿用 `range` 查詢參數重新載入統計資料。
- 分類標題按鈕由左至右切換「總覽、對戰統計、進攻分析、母球控制、最近練習」，預設顯示最左側「總覽」；畫面一次只顯示作用中分類資料，按鈕不使用卡片背景、外框或圓角。
- 「總覽」與母球控制內的出手數標籤依時間選單顯示「今日出手數、近 7 天出手數、近 30 天出手數、近一年出手數」，資料則由對應 `range` API 結果更新。
- 分類按鈕使用 `15px * var(--ui-font-scale)`、48px 最小高度、12px 按鈕間距與水平留白，按鈕字級必須跟隨使用者介面文字大小設定。
- 「最佳連進」與「訓練完成率」歸入總覽；母球控制分類只顯示走位成功率、停點偏差、洗袋次數、下一球好打比例與目前時間範圍出手數。
- 時間下拉選單的原生選項使用深色背景與淺色文字，避免展開後出現白底選項面板。
- 進攻分析四組數據改為由上到下的全寬排列，移除卡片底色與外框；組內不加入額外分隔線。標籤、長條與數值欄位使用一致欄寬，避免 AI Coach 展開後內容重疊。
- 對戰統計、最近對戰、總覽、母球控制同步改為全寬資料列，不使用卡片底色、外框、圓角或陰影。
- 統計資料列標籤統一使用 `15px * var(--ui-font-scale)`，提升「總局數、勝場、表現分數」等欄位的可讀性。
- 「勝率、表現分數、總覽進球率、最常失誤」只顯示文字數值，不顯示進度長條；進攻分析的比例型指標維持原有長條。
- 左側區段標題與右側第一筆資訊使用相同頂端基準線；右側資料小標題靠左，數值靠右並與時間範圍下拉選單的右邊界對齊。
- 所有資料列固定使用「小標題、長條預留欄、數值」三欄；沒有長條時保留中間空間，避免小標題與數值過近或位置跳動。
- 768px 以下窄版使用可收縮三欄，避免 AI Coach 展開後標題被裁切；數值欄統一靠右並對齊內容右緣。
- 最近對戰整組移至對戰統計的右側資訊欄，勝負、對手與比分沿用相同三欄位置。
- 分類工具列使用單一底部分隔線，分類內容不重複顯示區段標題或卡片外框。
- 分析頁移除「趨勢」區段、日／週／月／年切換與趨勢圖；前端不再呼叫 `/api/analytics/trends`。
- 返回按鈕只在呼叫端提供 `onBack` 時顯示，不再為了頁首文案額外佔用高度。

### 規範用法

- 分析頁若由頂部導覽直接進入，首屏應直接顯示分類標題列、右側時間選單與作用中分類內容，不顯示額外頁首文案。
...
- 進攻分析資料群組應使用 `.analytics-data-stack` 單欄排列；`.analytics-row` 必須保留標籤、長條、數值三欄，數值靠右且不可覆蓋長條。
- 統計指標使用 `.analytics-page .analytics-metric-card` 三欄資料列；小標題固定第一欄、數值固定第三欄，沒有進度條時第二欄保持空白。
- `MetricCard` 只有在 `progress` 為數字時才渲染長條；不需要長條的指標不得傳入 `progress`。
- `MetricCard` 必須依 `progress` 自動套用 `.has-progress` 或 `.without-progress`，不可用個別欄位名稱判斷版面。
- 窄版 `.analytics-metric-card` 與 `.analytics-row` 的第三欄必須使用 `justify-self: end` 與右對齊，且第一欄允許收縮，不得以固定最小寬度造成水平溢位。
- 已移除的趨勢功能不得保留 `TrendPayload`、`trendBucket`、圖表元件或背景 API 請求。
- 最近對戰與練習紀錄可沿用 `.practice-item` 語意結構，但分析頁限定樣式不得恢復卡片背景、外框或陰影。
- 最近對戰容器使用 `.stats-cards + .recent-practice` 定位至右側欄位；`.practice-item` 的三欄尺寸必須與統計資料列一致。
...
<div className="stats-page friend-match-page analytics-page">
  <div className="friend-match-panel stats-panel">
    ...
    <nav className="stats-view-toolbar" aria-label="統計分類">
      <div className="stats-view-tabs" role="tablist">...</div>
      <select className="stats-range-select" value={range}>...</select>
    </nav>
    <section className="stats-view-section" role="tabpanel">...</section>
  </div>
</div>
```
...
- 統計頁段落標題前方不再顯示數字。
- 時間範圍可由下拉選單切換，切換後會重新發送對應 `range` 的 analytics 查詢。
- 進攻分析四組資料由上到下等寬排列，沒有卡片外框或組內分隔線；AI Coach 開啟時長條與數值不重疊。
- 對戰統計、最近對戰、總覽、母球控制與練習紀錄均以無卡片資料列顯示，數值右緣與時間範圍下拉選單一致。
- 「勝率、表現分數、總覽進球率、最常失誤」不得顯示長條圖，但仍保留中間欄位，確保小標題與其他資料列左緣一致。
- 768px 以下窄版不得裁切資料小標題，所有數值右緣必須一致。
- 最近對戰顯示於「對戰統計」分類內，勝負、對手與比分分別對齊第一、第二、第三欄。
- 左側區段標題須與右側首列頂端對齊；每列小標題、長條與數值不得上下錯列或對應到相鄰資料。
- `.analytics-data-stack` 與緊接區段標題的 `.recent-practice` 不得加入頂部位移，避免右側首筆資訊低於左側標題。
- 頁面應只渲染目前作用中的指定分類，點擊標題後對應資料必須取代原內容；DOM、網路請求中均不存在趨勢功能。
- 時間選單必須維持在分類工具列最右側，展開選項時不得出現白底白字或不可辨識文字。
...
- 後端會檢查 AI Coach 回傳的 `source` 是否為 `analytics_advice`。若遠端 AI Coach 尚未更新而回傳舊版產品導覽，後端會使用同一份 DB analytics context 產生相容回覆，不會再引導玩家前往回放記錄。
- analytics 串流會先驗證完整結果再送至前端，避免舊服務的錯誤導覽文字先被逐字顯示；手機端與桌面端不需增加版本判斷。
- 後端輸出層會將 `shot events`、`practice records` 等模型技術措辭轉為「出桿紀錄」、「練習紀錄」，前端不得直接顯示內部欄位名稱。
...
- AI Coach service 的 analytics 回覆 `source` 必須以 `analytics_advice` 開頭；其他來源視為舊版或不相容結果，由 FastAPI 相容層改用資料庫內容回覆。
...
- 遠端 `coach.lessleap.com` 即使仍是舊版，數據問題也不會回覆「請到回放記錄」或「點擊產生建議」。
- 問「這球怎麼打」仍走即時球桌畫面分析。
